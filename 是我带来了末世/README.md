...
│  └─ 15_lore_optimization_patch.md # 设定优化补丁：男主动机、去AI味、组织粗粝感
│
├─ prompts/                # 可复用提示词
...
当前版本新增 `docs/15_lore_optimization_patch.md` 作为设定优化补丁。写作时若该补丁与 `docs/00-14` 存在冲突，正文创作阶段优先遵守补丁规则。

它重点解决四类问题：

1. **男主制造末世的唯一性证明**：补强“为什么不能温和拯救，为什么必须让整个位面进入应激状态”。
2. **去 AI 味正文规则**：限制“不是A，是B”“他不知道”“没有人回答”等模板句式，减少内部文档词进入正文。
3. **力量体系非游戏化呈现**：不靠直接报等级制造爽点，而通过身体代价、环境反馈、旁人反应和副作用呈现成长。
4. **组织粗粝感**：公会、政府、军方、黑市、教团都必须有临时性、烂账、私心和错误，不得像一开始就设计好的制度机器。

## 推荐工作流

### 1. 初始化（已完成）
所有设定文档已在 `docs/` 目录中创建完毕，并新增设定优化补丁 `docs/15_lore_optimization_patch.md`。

### 2. 写新章节
```
请先读取 docs/15_lore_optimization_patch.md，再根据 docs/10_chapter_outline.md 中第X章的章纲，写出正文。
```

### 3. 修订章节
```
请根据 docs/13_quality_checklist.md 检查并修订 drafts/volume_XX/ch_XXXX.md，重点检查是否符合 docs/15_lore_optimization_patch.md。
```
...
### 5. 扩展到下一卷
```
请先读取 docs/15_lore_optimization_patch.md，再为第X卷生成详细大纲，写入 docs/08_volume_outline.md
```
...
3. 智能体会先读取 `docs/15_lore_optimization_patch.md`，再读取相关设定文档
...
- **不急着替男主辩护**：先让读者恨他、怕他、想知道他；后期再让读者理解“他未必无罪，但不愚蠢”

## 当前状态

- [x] 项目框架搭建完成
- [x] 创作圣经 v2.0 全部就绪（15 份核心文档）
- [x] 设定优化补丁 v1.0 已新增（`docs/15_lore_optimization_patch.md`）
